...
### 大小写
1. git 默认不区分大小写。
2. 如果只是改了文件或目录的大小写，那 git 并不会更新路径信息。这就导致它监听变化时监听的路径是错误的。
3. 遇到过一次是把目录 `huawei` 改成了 `Huawei`，然后在这里面代码变动不会有 gutter 高亮提示了。
4. 解决方法是删除旧的小写 `huawei` 目录，确保 Git 只跟踪新的大写 `Huawei` 目录：
    ```bash
    # Windows PowerShell
    Remove-Item -Path "d:\my_gits\AndroidThemes\huawei" -Recurse -Force

    # Linux/macOS
    rm -rf "d:/my_gits/AndroidThemes/huawei"
    ```
5. 然后把 git 改成区分大小写以防以后出现同样问题
    ```bash
    git config core.ignorecase false
    ```
6. 在不区分大小写的情况下，修改大小写要通过 git
    ```bash
    git mv huawei Huawei
    ```
