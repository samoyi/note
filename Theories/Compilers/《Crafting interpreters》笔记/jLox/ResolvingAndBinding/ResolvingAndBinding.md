<!-- vscode-markdown-toc -->
* 1. [Static Scope](#StaticScope)
	* 1.1. [Scopes and mutable environments](#Scopesandmutableenvironments)
	* 1.2. [Persistent environments](#Persistentenvironments)
* 2. [Semantic Analysis](#SemanticAnalysis)
	* 2.1. [A variable resolution pass](#Avariableresolutionpass)
* 3. [A Resolver Class](#AResolverClass)
	* 3.1. [Resolving blocks](#Resolvingblocks)
	* 3.2. [Resolving variable declarations](#Resolvingvariabledeclarations)
	* 3.3. [Resolving variable expressions](#Resolvingvariableexpressions)
	* 3.4. [Resolving assignment expressions](#Resolvingassignmentexpressions)
	* 3.5. [Resolving function declarations](#Resolvingfunctiondeclarations)
	* 3.6. [Resolving the other syntax tree nodes](#Resolvingtheothersyntaxtreenodes)
		* 3.6.1. [语句](#)
		* 3.6.2. [表达式](#-1)
* 4. [Interpreting Resolved Variables](#InterpretingResolvedVariables)
	* 4.1. [Accessing a resolved variable](#Accessingaresolvedvariable)
	* 4.2. [Assigning to a resolved variable](#Assigningtoaresolvedvariable)
	* 4.3. [Running the resolver](#Runningtheresolver)
* 5. [Resolution Errors](#ResolutionErrors)
	* 5.1. [Invalid return errors](#Invalidreturnerrors)
...
<!-- vscode-markdown-toc-config
	...
	autoSave=true
	/vscode-markdown-toc-config -->
<!-- /vscode-markdown-toc --># Resolving and Binding


##  1. <a name='StaticScope'></a>Static Scope
...
###  1.1. <a name='Scopesandmutableenvironments'></a>Scopes and mutable environments
...
###  1.2. <a name='Persistentenvironments'></a>Persistent environments
...
##  2. <a name='SemanticAnalysis'></a>Semantic Analysis
1. 目前，每次评估变量表达式时，我们的解释器都会解析变量，也就是追踪它引用的声明。如果该变量被包裹在运行一千次的循环中，那么该变量就会被重新解析一千次。
2. 上面说了，静态作用域意味着变量始终解析为相同的声明。所以，其实我们只需要解析一次就可以了。
3. 我们可以用很多方法来存储变量和其声明之间的绑定。在下篇 C 解释器的部分我们会实现一个更高效的存储和访问变量的方案，但这里我们不去做太多改变，将充分利用现有的 `Environment` 类的实现。
4. 再看看上面有问题的例子中两次对变量 `a` 的解析。第一次沿着环境链找了三个环境找到了 `a` 的声明，这是正确的解析；而第二次只找了两个环境就找到了，这是错误的解析。
5. 如果我们可以确保每次解析变量时查找的环境数量都和第一次的数量相同，那就可以保证每次都找的是同一个变量声明了，也就是正确的那一个。

###  2.1. <a name='Avariableresolutionpass'></a>A variable resolution pass
...
##  3. <a name='AResolverClass'></a>A Resolver Class
1. 新建解析器类。由于解析器需要访问语法树中的每个节点，因此它实现了我们已经拥有的访问者抽象
	```java
	// lox/Resolver.java
...
3. 其余节点没有做任何特殊的事情，但我们仍然需要为它们实现访问方法，以便遍历它们的子树。比如即使 `+` 表达式本身没有任何变量需要解析，但它的任何一个操作数都可能有。

###  3.1. <a name='Resolvingblocks'></a>Resolving blocks
1. 解析块的 visit 方法如下，这个方法开启一个新的作用域，遍历块内的语句，然后丢弃该作用域
	```java
	// Resolver.java
...
9. 作用域栈仅用于本地块作用域。全局作用域顶层声明的变量不会被解析器跟踪，因为它们在 Lox 中更具动态性。解析变量时，如果我们在本地作用域栈中找不到它，我们会假设它一定是全局的。
...
###  3.2. <a name='Resolvingvariabledeclarations'></a>Resolving variable declarations
...
###  3.3. <a name='Resolvingvariableexpressions'></a>Resolving variable expressions
...
###  3.4. <a name='Resolvingassignmentexpressions'></a>Resolving assignment expressions
...
###  3.5. <a name='Resolvingfunctiondeclarations'></a>Resolving function declarations
...
###  3.6. <a name='Resolvingtheothersyntaxtreenodes'></a>Resolving the other syntax tree nodes
####  3.6.1. <a name=''></a>语句
...
##  4. <a name='InterpretingResolvedVariables'></a>Interpreting Resolved Variables
1. 每次访问变量时，解析器都会告诉解释器当前作用域和定义变量所在的作用域之间有多少个作用域。在运行时，这恰好对应于当前环境和解释器可以找到变量值的环境之间的环境数量。
2. 解析器通过调用以下命令将该数字传递给解释器
	```java
	// Interpreter.java
	...
	```
3. 我们需要将解析信息存储在某个地方，以便稍后执行变量表达式和赋值表达式时可以使用它。一个直观的存储的地方就是语法树节点本身，许多编译器都会将此类分析的结果存储在这个位置。
4. 我们可以这样做，但这需要对语法树生成器进行一些改动。而且存储在语法树中的话，如果语法树频繁的重建，对于很深的节点，重新计算它们的数据并不容易。
5. 这里我们采用另一种常见方法，将其存储单独的一个 map 中，其中映射了每个语法树节点和它对应的解析数据。我们这里并不会建立树形或者嵌套结构，所以会简单一些；而且如果想要清除解析信息，只要很方便的清除 map 就可以了。定义需要的 map
	```java
	// Interpreter.java
...
###  4.1. <a name='Accessingaresolvedvariable'></a>Accessing a resolved variable
...
2. `lookUpVariable` 方法
	...
	```
3. `lookUpVariable` 方法首先根据表达式 `expr` 从 `locals` 中找到该表达式之前解析到的环境个数距离。如果没找到，那就说明这个变量是全局变量，否则就使用 `getAt` 方法从根据距离和变量名查找该变量
	```java
	// Environment.java
...
###  4.2. <a name='Assigningtoaresolvedvariable'></a>Assigning to a resolved variable
...
###  4.3. <a name='Runningtheresolver'></a>Running the resolver
在 Lox 的 `run` 方法中运行解析器
```java
// Lox.java
...
##  5. <a name='ResolutionErrors'></a>Resolution Errors
...
###  5.1. <a name='Invalidreturnerrors'></a>Invalid return errors
1. 在函数体外不允许出现 `return` 语句，我们的解析器可以对此进行静态检查，我们可以跟踪当前访问的代码是否在函数声明内。
2. 定义一个枚举类型 `FunctionType` 以及一个该类型的变量 `currentFunction`，用来计算当前是否正在解析函数声明
	```java
	// Resolver.java
	...
	```
3. 当我们解析函数声明时，把这个枚举类型变量传给 `resolveFunction`
	```java
	// Resolver.java
	...
	```
4. 在 `resolveFunction` 中，我们首先将 `currentFunction` 字段的先前值存储在局部变量中 `enclosingFunction` 中，然后用参数 `type` 重设 `currentFunction`。等到解析完函数后，再把 `currentFunction` 恢复到函数外面对应的 `currentFunction` 值
	```java
	// Resolver.java
	...
	```
5. 现在我们可以随时判断我们是否处于函数声明中，我们会在解析 `return` 语句时检查这一点
	```java
	// Resolver.java
