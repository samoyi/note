...
export function initEvents (vm: Component) {
    // 为每个实例创建一个 _events 对象
    // 该对象将保存绑定于其上的自定义事件。对象的 key 为自定义事件的名称，对应的 value 是一个数组，包含该事件的所有处理函数
    vm._events = Object.create(null)
    // 用于标记该实例是否绑定了对生命周期`hook:`函数的监听，见下
    vm._hasHookEvent = false
    // 父组件通过指令添加的自定义事件监听。
    // 结构为对象。因为通过指令不能为一个事件绑定多个回调，事件名 key 对应的 value 是一个函数而非数组。
...
export function eventsMixin (Vue: Class<Component>) {
    ...
    Vue.prototype.$once = function (event: string, fn: Function): Component {
        ...
        // 回调数组里是否有要解除的回调，还要同时检查回调数组里是否某个回调拥有 fn 属性且该属性恰好就是要接触的回调
        on.fn = fn
        // 为事件绑定包装回调
        vm.$on(event, on)
        return vm
    ...
    Vue.prototype.$off = function (event?: string | Array<string>, fn?: Function): Component {
        ...
        if (fn) {
            ...
            while (i--) {
                cb = cbs[i]
                // 如果某个回调就是要解绑的回调，或者该回调是要解绑的 $once 回调，则从指定事件对应的回调数组中删除该回调并跳出循
                // 环。
                if (cb === fn || cb.fn === fn) {
                    cbs.splice(i, 1)
                    break
