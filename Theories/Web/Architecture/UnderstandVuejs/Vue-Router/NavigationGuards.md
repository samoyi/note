...
3. 如果创建了多个该方法，则按照创建顺序调用。守卫是异步解析执行，此时导航在所有守卫resolve 完之前一直处于等待中。类似于`Promise.all`的感觉，不知道源码是怎么实现的。不懂
...
2. 进入（enter）每条路由前会触发。
3. 下面的两个 route 对应两个组件，在组件间导航时，每次都会从一条 route 进入另一条route，每次进入前，就会触发被进入 route 的`beforeEnter`方法。
