...
function observe(self) {
    ...
    Object.keys(data).forEach(function(key) {
        defineReactive(data, key, data[key], self);
    });
};
 ...
 * 顺便添加 watch 功能
 */
function defineReactive(data, key, val, self) {
    const publisher = new Publisher();
    Publisher.curPub = publisher;

    // 当前属性是否被 watch
    let bHasWatcher = (self.watch && (typeof self.watch[key] === 'function'));
    ...
    Object.defineProperty(data, key, {
        ...
        set(newVal) {
            ...
            if (bHasWatcher){ // 如果被 watch
                self.watch[key](newVal, val);
            }
