...
用绝对路径，HMTL 和 CSS 中引用的资源都可以被打包。详见`vue-loader`[文档](https://vue-loader-v14.vuejs.org/zh-cn/configurations/asset-url.html)。
...
安装`url-loader`。如果安装了`url-loader`就不需要再安装`file-loader`，如果资源超过了
设定的限制值而不能转换为 DataURL 时，`url-loader`会使用其内部的`file-loader`加载。
...
## 编写入口文件和 vue 根级组件
1. 入口文件为`src/index.js`，为 webpack 构建依赖图的入口，也是创建 vue 根实例的地方
```js
// src/index.js
import Vue from 'vue';

// Vue 根级组件，是 vue 根实例的唯一子级组件，是其他所有实际编写组件的容器
import App from './App';
