...
- [Gradient Descent](#gradient-descent)
    ...
    - [梯度下降法的含义与公式](#梯度下降法的含义与公式)
        - [梯度下降法的思路](#梯度下降法的思路)
        - [用向量表示函数的变化值](#用向量表示函数的变化值)
        - [二变量函数的梯度下降法的基本式](#二变量函数的梯度下降法的基本式)
            - [一例](#一例)
        - [梯度下降法及其用法](#梯度下降法及其用法)
        - [将梯度下降法推广到三个变量以上的情况](#将梯度下降法推广到三个变量以上的情况)
        - [哈密顿算子 $\nabla$](#哈密顿算子-\nabla)
        - [$\eta$ 的含义以及梯度下降法的要点](#\eta-的含义以及梯度下降法的要点)
...
3. 在函数的情况下也完全一样。要寻找函数的最小值，可以利用上面的结论，根据当前变量值的偏导找出当前使函数值减小得最快的向量 $(\Delta x,\Delta y)$ 并移动。在移动后到达的点处，再次根据当前新的变量值的偏导计算当前使函数值减小的最快的向量 $(\Delta x,\Delta y)$ 并移动。通过反复进行这样的计算和移动，就可以找到最小值点。这种寻找函数 $f(x,y)$ 的最小值点的方法称为二变量函数的**梯度下降法**。
