...
- [Basic](#basic)
    ...
    - [6 梯度](#6-梯度)
        ...
        - [6.5 梯度法](#65-梯度法)
            - [6.5.1 数学表达](#651-数学表达)
            - [6.5.2 Python 实现梯度下降法](#652-python-实现梯度下降法)
            - [6.5.3 实例](#653-实例)
        - [6.6 神经网络的梯度](#66-神经网络的梯度)
            - [6.6.1 实例：实现一个简单的神经网络并求梯度](#661-实例实现一个简单的神经网络并求梯度)
    - [7 学习算法的实现](#7-学习算法的实现)
        - [7.1 实现 2 层神经网络（隐藏层为 1 层的网络）的类](#71-实现-2-层神经网络隐藏层为-1-层的网络的类)
        - [7.2 mini-batch 的实现](#72-mini-batch-的实现)
        - [7.3 基于测试数据的评价](#73-基于测试数据的评价)
            - [7.3.1 需要确认泛化能力](#731-需要确认泛化能力)
            - [7.3.2 确认泛化能力的代码实现](#732-确认泛化能力的代码实现)
...
3. 神经网络对正确的选项给出的概率越高（越接近 1），对数的值（负值）就越接近 0，损失函数的结果作为该负值的绝对值也就越小。
