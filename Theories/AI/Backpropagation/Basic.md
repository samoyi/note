...
- [Basic](#basic)
    ...
    - [6.Affine/Softmax 层的实现](#6affinesoftmax-层的实现)
        ...
        - [6.3 Softmax-with-Loss 层](#63-softmax-with-loss-层)
            - [6.3.1 Softmax-with-Loss 层的实现](#631-softmax-with-loss-层的实现)
...
## 1. 数值微分求梯度的局限
1. 通过数值微分计算神经网络的权重参数的梯度虽然简单，也容易实现，但缺点是计算上比较费时间。实际上，在 `Theories/AI/Perceptron/Basic.md` 中说到，神经网络曾经就是因为过于复杂的导数计算而陷入停滞。直到后来，本章要讲的反向传播算法的提出，才再次启动了神经网络的研究。
