- [思想](#思想)
  - [正向与反向](#正向与反向)
- [数值微分求梯度的局限](#数值微分求梯度的局限)
- [计算图](#计算图)
  - [为什么使用计算图](#为什么使用计算图)
  - [用计算图求解问题实例](#用计算图求解问题实例)
  - [局部计算](#局部计算)
  - [为何用计算图解题](#为何用计算图解题)
- [反向传播](#反向传播)
  - [计算图的反向传播](#计算图的反向传播)
  ...
  - [乘法节点的反向传播](#乘法节点的反向传播)
  - [苹果的例子](#苹果的例子)
  - [反向传播为什么能求导](#反向传播为什么能求导)
    - [实际上就是复合函数求导](#实际上就是复合函数求导)
- [简单层的实现](#简单层的实现)
  - [乘法层的实现](#乘法层的实现)
  - [加法层的实现](#加法层的实现)
- [激活函数层的实现](#激活函数层的实现)
  - [Sigmoid 层](#sigmoid-层)
    ...
    - [反向传播步骤](#反向传播步骤)
      ...
      - [步骤3](#步骤3)
      - [步骤4](#步骤4)
    - [Python 实现 Sigmoid 层](#python-实现-sigmoid-层)
- [Affine/Softmax 层的实现](#affinesoftmax-层的实现)
  - [Affine 层](#affine-层)
    - [正向传播](#正向传播)
    - [看一下反向传播的过程](#看一下反向传播的过程)
  - [批版本的 Affine 层](#批版本的-affine-层)
    - [偏置的求导](#偏置的求导)
    - [Affine 的实现](#affine-的实现)
  - [Softmax-with-Loss 层](#softmax-with-loss-层)
    - [Softmax 层计算图](#softmax-层计算图)
    - [Softmax-with-Loss 层计算图的推导](#softmax-with-loss-层计算图的推导)
      - [Softmax 层正向传播](#softmax-层正向传播)
      - [Cross entropy error 层正向传播](#cross-entropy-error-层正向传播)
      - [Cross entropy error 层反向传播](#cross-entropy-error-层反向传播)
      - [Softmax 层反向传播](#softmax-层反向传播)
        - [步骤 1](#步骤-1)
        - [步骤 2](#步骤-2)
        - [步骤 3](#步骤-3)
        - [步骤 4](#步骤-4)
    - [对反向传播结果的分析](#对反向传播结果的分析)
    - [Softmax-with-Loss 层的实现](#softmax-with-loss-层的实现)
- [误差反向传播法的实现](#误差反向传播法的实现)
  - [神经网络学习的全貌图](#神经网络学习的全貌图)
  - [对应误差反向传播法的神经网络的实现](#对应误差反向传播法的神经网络的实现)
  - [误差反向传播法的梯度确认](#误差反向传播法的梯度确认)
  - [使用误差反向传播法的学习](#使用误差反向传播法的学习)
# Basic
