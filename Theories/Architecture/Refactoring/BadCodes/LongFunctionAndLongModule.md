...
### 意图和实现分离
1. 要让阅读代码的人在不需要修改而只是阅读一段代码时，可以立刻知道它的意图是什么，而不用去关心它是怎么实现的。
2. 要做到这一点，就要让使用若干个符合 SRP、命名良好的小函数，而不能是一个大函数。

### 隔离变化
一段逻辑处理若干个子逻辑，并且有些子逻辑可能变动频繁有些子逻辑补偿变动，那拆分之后，对频繁变动的那部分修改就不会影响到不经常变动的那部分。
