...
## TODO
* 无环依赖原则(ADP)，参考[这篇](https://zhuanlan.zhihu.com/p/78472109)
...
## 评估
### 组件粒度
* 如果一部分代码可能会被用在好几处不同的地方，那就可以考虑作为单独的组件。
* 如果一部分代码相比于它周围的代码有更高的可能型会被频繁修改，且修改的时候影响到周围的代码，那就可以考虑作为单独的组件，把和周围代码的相互影响作为 prop 和事件。
* OCP：一部分代码倘若分离的话，是否打破了一个逻辑上有意义的实体，如果是的话，那就要考虑是否值得分离。
...
## 命名
1. 考虑 *Change Function Declaration* 重构原则。
2. 如果是业务组件，那命名就应该和业务相关；如果是基础组件，那就应该和业务无关。
3. 如果发现没法只用一个概念来命名组件，那可能就意味着这个组件可以拆成两部分。
...
使用 mixin 和 store
### 组件不应该有副作用
* 如果一个组件是可以跨项目复用的组件，那么它对外部唯一的影响就只是向父级 emit 事件。
* 如果一个组件是只在当前项目复用的组件，那么它对外部唯一的影响还可以和 store 交互。
* 如果一个组件只是作为一个大组件的子组件，它本身并不会被复用，那么它的影响范围可以扩展到这个大组件内部。例如通过 mixin 的方法修改大组件内的公共数据。



### 不要勉强复用
不要为了复用把一个组件做的又丑又难理解，组件化的设计思想之一是小而美。
...
## data 设计
### data 属性应该存放什么数据
* **原始数据**：考虑到有计算属性的存在，如果一个数据可以由另一个 data 属性变换得到，那么这个数据就不应该作为一个 data 属性。
* **共享数据**：如果同辈组件拥有相同的 data 属性，那么这个 data 属性应该放到统一的外部来源——父级、mixin 或者 store。
* **非响应式数据**：data 属性都会被转换为响应式的，所以如果一个数据没有响应式需求（不作为依赖），或者干脆是不变的，那就没必要再为它添加响应式。



### data 正确的反映数据结构
1. 考虑 *Introduce Parameter Object* 和 *Split Parameter Object* 这两个相反的重构规则。
2. 如果 data 越不扁平，使用起来肯定会麻烦一些在读取、设置和监听时，需要一层一层的取属性。例如
    ```js
    data () {
        return {
            group: {
...
3. 但是，**data 作为一个组件的数据源，它应该如实反映组件数据的结构**。所以，不应该为了扁平而破坏数据的逻辑结构。
