...
4. 提供给使用者一个命令绑定接口，让它选择使用某个命令实例，绑定到自己的行为上。
...
* 命令绑定接口：提供给命令发起者，命令发起者使用该接口执行某个命令实例来实现需要的功能。对应这里的 `setCommand` 函数。

### 五类对象的耦合关系
#### 命令发起者和命令绑定接口
1. 命令绑定接口需要知道把命令绑定到命令发起者的哪个事件上，所以命令绑定接口要耦合命令发起者的某个事件。
2. 这可能需要针对不同的事件类型实现不同的的命令绑定接口。
