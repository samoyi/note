...
- [Basic](#basic)
    - [抽象本质](#抽象本质)
        ...
        - [七桥问题的抽象本质](#七桥问题的抽象本质)
    - [运用余数寻找周期性](#运用余数寻找周期性)
        - [今天是星期天，$10^{100}$ 天后是星期几？](#今天是星期天10^100-天后是星期几)
        - [求 $1234567^{987654321}$ 的个位数](#求-1234567^987654321-的个位数)
    - [寻找奇偶规律](#寻找奇偶规律)
        - [寻找恋人问题](#寻找恋人问题)
            - [现在试着简化节点数](#现在试着简化节点数)
        - [铺设草席问题](#铺设草席问题)
...
## 抽象本质
### 余数和周期性
1. 周期性的美妙之处在于，我们看似面对了特别多的情况，但只要运用周期性分析就会发现，其实情况是很有限的。
2. 就像正弦函数，自变量可以传任意的、特别大的值，但其实也只是对 $2\pi$ 范围内的重复而已。
3. 所以，在面对一个很大的范围或者很大的数时，只要具有周期性，那问题就会立刻得到化简。
4. 而化简的方法，就是通过余数。也就是这个值除以周期的余数。

### 余数就是分类
1. 分组就是根据事物的特征，把相同特征的分为同一类。
2. 一组数共同除以某个数的余数，就是这一组数的一个分类维度。
3. 最常见的分类就根据除以 2 的余数来做奇偶分类。

### 奇偶是余数的特例
1. 当除数是 2 时，余数问题就成了奇偶问题。
2. 根据余数的周期性分组就变成了奇偶分组。
...
3. $10^{100}$ 天后星期几和 $1234567^{987654321}$ 求个位数的问题也是同样的思路。
...
## 运用余数寻找周期性
### 今天是星期天，$10^{100}$ 天后是星期几？
1. 如果通过除以 7 来计算，并不可行，因为数字太大了。
2. 一时是得不出解析性的答案的。所以还是要先通过简化数量，递归性的由简单到难的寻找规律。
3. 可能会想到好几种简化方法，都试试，然后就会发现，不断尝试 $10^n / 7$ 的这种递归会找到规律性。

### 求 $1234567^{987654321}$ 的个位数
1. 有了上面思路，可能想进行 $1234567^n$ 这样的尝试。不过发现底数太大了行不通。
2. 不过题目既然说了是个位数，那其实就是其他位没关系，所以就尝试 $7^n$ 吧，于是很快找到了规律。
