...
- [Summary](#summary)
    - [CPU 的内部结构解析](#cpu-的内部结构解析)
    - [CPU 是寄存器的集合体](#cpu-是寄存器的集合体)
        - [寄存器的主要种类和功能](#寄存器的主要种类和功能)
        - [CPU 指令的大致类型](#cpu-指令的大致类型)
    - [决定程序流程的程序计数器](#决定程序流程的程序计数器)
    - [条件分支和循环机制](#条件分支和循环机制)
        - [标志寄存器](#标志寄存器)
    - [函数的调用机制](#函数的调用机制)
...
4. 另外简单地解释一下内存。通常所说的内存指的是计算机的 **主存储器**（main memory）3，简称主存。主存通过控制芯片等与 CPU 相连，主要负责存储指令和数据。主存由可读写的元素构成，每个字节都带有一个地址编号。CPU 可以通过该地址读取主存中的指令和数据，当然也可以写入数据。
...
3. CPU 中每个寄存器的功能都是不同的。用于运算的数值放在累加寄存器中存储，表示内存地址的数值则放在基址寄存器和变址寄存器中存储。上面汇编代码中用到的 `eax` 和 `ebp` 分别是累加寄存器和基址寄存器。
...
<table width="90%" border="1">
    ...
    <tbody>
        <tr>
            <td>数据转送指令</td>
            <td>寄存器和内存、内存和内存、寄存器和外围设备<span class="注释编号">17</span>之间的数据读写操作</td>
        </tr>
        <tr>
            <td>运算指令</td>
...
## 决定程序流程的程序计数器
1. 用户发出启动程序的指示后，操作系统会把硬盘中保存的程序复制到内存中。下图是实例程序启动时内存内容的模型，示例程序实现的是将 123 和 456 两个数值相加，并将结果输出到显示器上
