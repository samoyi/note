...
## 注意点
### 对输入数组乱序
这能够防止出现最坏情况并使运行时间可以预计。Hoare 在 1960 年提出这个算法的时候就推荐了这种方法——它是一种（也是第一批）偏爱随机性的算法。
...
1. 说明 `j` 停留在最右边没有动，`i` 一直走到了 `j` 的位置。如果这是不通过 `break` 跳出，而是再次试图循环，循环条件 `arr[i] < pivot` 可能成立也可能不成立。
