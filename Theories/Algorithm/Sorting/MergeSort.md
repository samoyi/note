...
4. 下面的方法将两个排好序的子数组组成的一个大数组归并为整体有序的数组
    ```js
    function merge ( arr, low, mid, high ) {
        ...
        for ( let k=low; k<=high; k++ ) { // 这个循环保证了前两个 if 不会是同时排完的情况
    ...
    // [2, 6, 10, 17, 21, 26, 27, 29, 33, 35, 37, 38, 42, 50, 55, 67, 68, 79, 91, 94]
    ```
5. 虽然不懂这里说的抽象化是什么意思，不过，现在已经不是把两个数组归并为一个了，而是把一个数组里两个独立有序的部分原地归并排序了。
6. 也就是说，相比于上面归并的方法，这里不需要每次都创建一个 `left` 和 `right` 数组了。（`aux` 只是临时数组，`merge` 调用结束后就会被回收）
