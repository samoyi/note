...
- [HashTable](#hashtable)
    ...
    - [处理冲突——开放寻址法](#处理冲突开放寻址法)
        ...
        - [开放寻址散列的分析](#开放寻址散列的分析)
    - [分析散列搜索算法（TODO，和前面的分析整合）](#分析散列搜索算法todo和前面的分析整合)
...
## 关键细节
### 数组长度的选择
1. 使用求模法散列整数时，数组长度 M 不能选择 10 的 n 次幂作为数组长度，否则所有数只能通过后 n 位来区分，更高的数位完全不能用来区分。
2. M 应该选择质数。因为如果不是质数，只要输入数据大体上以 M 的某个因数呈现周期性，也就更容易在整体上呈现周期性。因数更小，输入数据也就更容易对它呈现周期性。选择质数的话就避免了因数的情况。
3. 大小元素总数的 1.3 倍左右。

### 链接法时间复杂度
时间复杂度是 $Θ(α+1)$，而因为我们确定的槽位数会和总元素数成正比，所以 $α$ 是常数，因此复杂度
简化为 $Θ(1)$。
...
### 载荷因子
1. 槽位占用率被称作 **载荷因子**，记作 $λ$。
2. 散列表的槽位数应该为元素总数的 1.3 倍左右。
...
10. 但还有一种情况下，周期也会是比较短的，那就是输入数据组成接近于以 $M$ 的因数为公差的等差数列。比如 $M$ 有一个因数是 $q$，那以此为公差的输入只要 $M/q$ 次就能完成一个周期，进而散列出相同的结果。
11. 例如上面第一个表格中，$M$ 是 8，它有因数 4，而数据的公差正好就是 4，所以只要两次就能完成一个周期。
12. 下面是一个数据更多的例子。$M$ 为 35，它的两个因数是 5 和 7 两个质数。数据跨度为 2 的和 5 呈现周期性，那只要 7 次这样为 2 跨度就可以和 35 呈现周期性（5*7）。而如果是对更小的 29 求模，周期性就没有那么强
    ```
    数据：30 31 35 36 40 41 45 46 50 51 55 56 60 61 65 66 70 71 75 76 80 81 85 86 90 91 95 96 100
    %35： 30 31 0  1  5  6  10 11 15 16 20 21 25 26 30 31 0  1  5  6  10 11 15 16 20 21 25 26 30
    %29： 1  2  6  7  11 12 16 17 21 22 26 27 2  3  7  8  12 13 17 18 22 23 27 28 3  4  8  9  13
    ```
13. 数据中出现等差数列也是挺常见的，例如 $2^n$ 就很普遍。所以在不确定输入的情况下为了让这里的周期性尽量的长，就应该使用质数作为 $M$，避免 $M$ 的因数符合数据中的规律。
### 散列浮点数
...
4. 因此，查找失败时会遍历整个链表，再加上之前计算散列值，因此时间复杂度为 $Θ(α+1)$；查找成功时，根据《算法导论》146页上没看懂的推导，时间复杂度仍然为 $Θ(α+1)$。
...
## 处理冲突——开放寻址法
### 探查序列
1. 为了使用开放寻址法插入元素，我们需要连续的检查散列表，直到找到一个空槽我。这个过程也叫作 **探查**（probe）。而我们将要探查的那些槽位所组成的序列，就被称为 **探查序列**（probe sequence）。
2. 探查序列并不一定是完全确定的序列，而是要依赖于待插入的 key。
3. 为了得到，我们的散列很
...
2. 在这种情况下，搜索事件不再依赖于 $O(α)$ 了，因为 $O(α)$ 的分子是实际的元素数，但是搜索时除了要跳过实际元素，还要跳过元素已被删除的槽位。
3. 因此，在必须要删除关键字的应用中，更常见的做法是采用链接法来解决冲突。

### 均匀散列
1. 对于一个拥有 $m$ 个槽位的哈希表，为了插入一个元素，它需要探查的可能性有多少种？
2. 第一次散列有 $m$ 种可能性，如果第一次没找到空位，再散列有 $m-1$ 种可能性，如果还没中，再散列有 $m-2$ 种可能性。以此类推，一个元素探查的可能性一共有 $m!$ 种。
...
3. 其实就是把线性探查再散列的 $C_1$ 从常数变成了函数 $h_1(k)$。也就是再散列时加上的数不是提前确定的，而是在运行时根据参数 $key$ 通过函数 $h_1$ 计算才确定的。
...
3. 平均情况下，插入一个元素至多需要 $1/(1-α)$ 探查。因为要插入就要先探查到一个空槽，这就相当于一次失败的查找。
4. 对于一次成功的查找，探查次数至多为 $\frac{1}{α} \ln\frac{1}{1-α}$。证明 TODO。
5. 同样，因为槽位数量 $m$ 和元素数量 $n$ 成正比，所以 $α$ 为常数，因此以大 $O$ 来表示，上述三个操作的时间复杂度都是 $O(1)$。


## 分析散列搜索算法（TODO，和前面的分析整合）
