...
3. 注意，“跨步”（skip）的大小要能保证表中所有的槽最终都被访问到，否则就会浪费槽资源。要保证这一点，常常建议散列表的大小为质数。互质也是可以的，例如散列表大小为 10，skip 为 3。上面基础的线性探测的跨步值为 1，而 1 就是和任何数互质的。TODO 质数原理。
3. 同样探查可能性只有 $m$。因为跨步是固定的，所以第一次散列值确定后后面整个探查序列就确定了。
...
4. 《算法导论》上的实现更复杂一些，探查序列是 $初始散列值 + C_1 * i^2 + C_2 * i$。$C_1$、$C_2$ 是常数，$i=0, 1, ..., m-1$。其实上面简单实现中，就是相当于 $C_1 = 1$、$C_2 = 0$。
5. TODO，这里同样要选择合适的 $C_1$ 和 $C_2$ 来保证可以散列到所有插槽。《算法导论》思考题 11-3 有这个题目，没看懂，不过下面系数选择里提供了几种可用方案。
6. 因为每次再散列不再像线性那样是到下一个相邻的槽位，而是跨度逐渐变大，所以不会出现很明显的集群。但是这里同样的，两个关键字的初始探查位置相同，它们的探查序列也相同，所以仍然会导致轻度的群集，称为 **二次群集**（secondary clustering）。
7. 实现在 `./QuadraticProbing.c`。
8. 虽然这里再散列改成了完全平方数，但因为也是确定的常数，所以探查可能性仍然是 $m$ 种。

#### 系数选择
1. 第一种方案很简单，就是上面的简单实现，跨步是一系列连续的完全平方数；同时保证散列表的槽位数是质数即可。
2. 第二种方案是让跨步为 $(i^2 + i) / 2, \enspace i=0, 1, ..., m-1$，（相当于 $C_1 = C_2 = 1/2$）同时让散列表槽位数是 2 的幂。TODO，互质？槽位数定为 2 的幂，在载荷到达阈值的时候扩建哈希表时可以直接很简单的翻倍，保证了槽位数仍然是 2 的幂。
...
6. 实现在 `DoubleHashing.c`。TODO，怎么确定遍历完了所有槽位
