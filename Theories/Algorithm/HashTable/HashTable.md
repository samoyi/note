...
3. 注意，“跨步”（skip）的大小要能保证表中所有的槽最终都被访问到，否则就会浪费槽资源。要保证这一点，常常建议散列表的大小为质数。互质也是可以的，例如散列表大小为 10，skip 为 3。TODO 质数原理。
...
4. 《算法导论》上的实现更复杂一些，探查序列是 $初始散列值 + C_1 * i + C_2 * i^2$。$C_1$、$C_2$ 是常数，$i=0, 1, ..., m-1$。
5. TODO，这里同样要选择合适的 $C_1$ 和 $C_2$ 来保证可以散列到所有插槽。《算法导论》思考题 11-3 有这个题目。
