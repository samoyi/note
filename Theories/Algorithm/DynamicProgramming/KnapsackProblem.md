...
2. 如果还有若干件物品时，解的形式为 $max(选出的某一件物品的价值 + 对剩余物品递归计算得到的价值)$。
...
4. 上面的递归式看起来没有问题，但是求解的时候却遇到了问题，很难写出表示子问题的二维数组
    * 第一种写法
        ...
        int knapsack_01 (int* weightIndexes, int volume) {
            for (int v=1; v<=V; v++) {
                int maxVal = 0;
                int currVal = 0;
                for (int i = 0; i<=N; i++) {
                    ...
                    // 第二个下标要怎么写才能表示出了第 i 个物品以外的其他物品
                    currVal = values[i] + table[v-weights[i]][这个下标要怎么写];
                    if (currVal > maxVal) {
                        maxVal = currVal;
                    }
                }
        ...
        int knapsack_01 (int* weightIndexes, int volume) {
            for (int v=1; v<=V; v++) {
                ...
                for (int i = 0; i<=N; i++) {
                    ...
                    // table[V-weights[i]][i] 表示在 V-weights[i] 容量，且物品种类不包含第 i 个的情况下，
                    // 所能装的最大价值
                    currVal = values[i] + table[v-weights[i]][i];
                    if (currVal > maxVal) {
                        maxVal = currVal;
                    }
                    table[v][这个下标要怎么写] = maxVal;
                }
            }
        }
        ```
5. 在之前分析最优子结构的时候，都是先选定一个元素，然后再对剩下的元素递归求解。但在这里这个方法却遇到了问题。
6. 0-1 背包问题用到的递归式是：$max(不装入当前物品的话的最大价值, 装入当前物品的话的最大价值)$
