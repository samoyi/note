...
## 新的最优子结构
1. 最优解的递归式，是关于 “做出某种选择” 的描述。上面有问题的递归式的选择是 “选出某一件物品”，由此得出的最优子结构递归式有问题。
2. 因此现在就要从其他方面考虑另一种选择。其实在钢条切割问题中，最初的到的最优子结构虽然正确但比较麻烦，所以也是进行了简化而得到了新的最优子结构。也就是说对于一个问题，最优子结构可能并不止一种，有些不可行，有些比较麻烦。
3. “原问题的解决包含做出某种选择，这种选择会将原问题划分为一个或多个子问题。”
4. 钢条切割问题的选择是：每一英寸处都有切或者不切两种选择；同样在这里，选择是：每一个物品都有选或不选两种选择。
5. 在不同的位置将原钢条一分为二，就会得到不同的两个子问题；选择拿不同的物品，则剩下的物品和容量就会得到一个子问题。怎么又绕回到上面的最优子结构了？
6. 实际上，背包问题的选择是，拿不拿第 N 件物品。这种情况下，选择的选项不是 N 个，而是两个。
7. “为了得出最优解而要做出的选择，肯定是所有选择的可能性中的一种，因此需要遍历所有选择的情况。”
8. 这里就两种选择，选或者不选第 N 个。两种选择会得到一个子问题：如果选了，子问题就是剩下的物品和减去物品 N 所需容量后剩余的背包容量；如果没选，子问题就是剩下物品和全部的背包容量。
9. 如果第 N 个物品的 $价值/重量$ 更高的话，那选它可能最终的总价格就更高；否则如果它价值不高但很重，选它就还不如让出空间以便从剩余的物品里能选出 $价值/重量$ 更高的。
