...
- [拼接和延长字符串](#拼接和延长字符串)
  - [`concat()`](#concat)
    - [和 `+` 转字符串的不同](#和--转字符串的不同-1)
  - [`padStart()` 和 `padEnd()`](#padstart-和-padend)
    - [Syntax](#syntax)
    - [`targetLength` 参数](#targetlength-参数)
    - [可选的 `padString` 参数](#可选的-padstring-参数)
  - [`repeat()`](#repeat)
    - [Syntax](#syntax-1)
    - [`count`参数](#count参数)
    - [不规范参数](#不规范参数)
- [字符串转数组](#字符串转数组)
  - [`split`](#split)
    - [Syntax](#syntax-2)
    - [可选的 `separator` 参数](#可选的-separator-参数)
    - [可选的 `limit` 参数](#可选的-limit-参数)
- [确定/查找子字符串](#确定查找子字符串)
  - [`includes()`](#includes)
...
- [获取、删除和替换子字符串](#获取删除和替换子字符串)
  ...
  - [替换和删除子字符串](#替换和删除子字符串)
    ...
    - [`String.prototype.replaceAll(pattern, replacement)`](#stringprototypereplaceallpattern-replacement)
- [格式化字符串](#格式化字符串)
...
#### `trim()` 局限
1. 有些字符在很多地方都看不见，但它不是空白字符。零宽空格 `​`(你在这里应该看不到 `` 之间的字符，但把它复制到 Chrome 的 Console 里可以看到，它是字符 `\u200B`) 不属于常规空白符。
2. 对于这样的字符，`trim()` 是无法去除的，即使它位于字符串的首尾。
3. 这种情况可以使用如下正则替换 `const cleaned = str.replace(/[\p{Cf}\p{Cc}\p{Cn}]/gu, '')`
4. 但它的作用不会覆盖 `trim`。
