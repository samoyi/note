...
## 1. Default binding
1. 默认的普通的函数调用情况下，`this` 并不会和变量一样拥有作用域链可以继承外部函数的 `this`。
2. 它永远只有两个值，要么是严格模式下的 `undefined`，要么是非严格模式下的全局对象
    ```js
    function getThis () {
        return this;
    }
...
### 在非全局严格模式的情况下，有一点需要注意
    ...
    ```
1. 之所以是 `window` 的原因，可以从下面的规范摘抄中得到解释：   
...
2. 一个函数是否按照严格模式来执行，不是看它的调用环境，而是看它的定义环境以及它本身函数体内是否是严格模式。
3. IIFE 中是严格模式，在它里面对 `this` 求值的结果是 `undefined`。但 `foo` 函数内部不是严格模式，所以对 `this` 求值时，因为不是方法调用且没有指定 `this`，所以 `this` 就被自动设定为全局对象。
