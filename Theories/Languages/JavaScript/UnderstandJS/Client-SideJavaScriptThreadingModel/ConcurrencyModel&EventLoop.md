...
4. 分析下面代码
    ...
    ```
5. 完整的执行流程：
    1. 引擎将全局执行环境推入调用栈，并在执行上下文中创建一个空的微任务队列；
    2. 调用 `foo`：
        1. 打印 `"foo"`;
        2. 调用 `Promise.resolve` 创建并返回一个 promise，该 promise 立刻出结果，结果为空值；
        3. 返回的 promise 调用 `then`，传递了该 promise 的回调函数，该回调会打印 `"micro-foo"`;
        4. 因为该 promise 已经出了结果，所以该回调立刻被推入微任务队列;
        5. 执行 `setTimeout`，给浏览器提交一个宏任务，该宏任务之后会打印 `"macro-foo"`；
        6. 调用 `bar`；
            1. 打印 `"bar"`;
            2. 调用 `Promise.resolve` 创建并返回一个 promise，该 promise 立刻出结果，结果为空值；
            3. 返回的 promise 调用 `then`，传递了该 promise 的回调函数，该回调会打印 `"micro-bar"`。
            4. 因为该 promise 已经出了结果，所以该回调立刻被推入微任务队列;
            5. 执行 `setTimeout`，给浏览器提交一个宏任务，该宏任务之后会打印 `"macro-bar"`；
        7. `bar` 调用结束；
    3. `foo` 调用结束；
    4. 打印 `"global"`；
    5. 调用 `Promise.resolve` 创建并返回一个 promise，该 promise 立刻出结果，结果为空值；
    6. 返回的 promise 调用 `then`，传递了该 promise 的回调函数，该回调会打印 `"micro-global"`;
    7. 因为该 promise 已经出了结果，所以该回调立刻被推入微任务队列;
    8. 执行 `setTimeout`，给浏览器提交一个宏任务，该宏任务之后会打印 `"macro-global"`；
    9. 全局执行环境里的所有函数执行完成；
    10. 调用栈清空前，开始执行微任务队列里的是哪个微任务：
        1. 第一个微任务打印 `"micro-foo"`；
        2. 第二个微任务打印 `"micro-bar"`；
        3. 第三个微任务打印 `"micro-global"`；
    14. 调用栈清空，开始执行事件队列里之后的宏任务：
        1. 第一个宏任务打印 `"macro-foo"`；
        2. 第二个宏任务打印 `"macro-bar"`；
        3. 第三个宏任务打印 `"macro-global"`；   
