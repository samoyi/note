...
### 列表转树
#### 最直观的实现
1. 最直观的实现就是遍历每一项，然后插入到其父级的的 `children` 数组里。
2. 两个特殊情况：
    * 如果当前项是根元素，不是插入，而是用一个 `root` 变量标记，之后返回
    * 如果插入时父级没有 `children` 则先创建
3. 实现
    ```js
    function list2tree(list) {
        ...
        list.forEach((item) => {
            if (item.pid === 0) {
                ...
                let p = list.find((innerItem) => {
                    return innerItem.id === item.pid;
                });
                if (!p.children) {
                    p.children = [];
                }
    ...
    console.log( JSON.stringify(list2tree(list), null, 4) )
    ```
4. 时间复杂度 $O(N^2)$。

#### 哈希实现
1. 哈希表的查找复杂度是 $O(1)$，所以可以用 $O(N)$ 的空间换时间。
2. 把元素都存进哈希表里，然后查找的时候从哈希表里查找
    ```js
    function list2tree(list, rootID) {
        let hash = {};
        list.forEach((item) => {
            item.children = [];
            hash[item.id] = item;
    ...
    console.log( JSON.stringify(list2tree(list, 1), null, 4) )
    ```
3. 时间复杂度 $O(N)$，空间复杂度也是 $O(N)$。

#### 如果元素 ID 是连续的
1. 那就可以先排序，然后遍历插入的时候就不用再搜索了，直接通过 ID 就能找到元素。
2. 这里数组 ID 是从 1 开始，和数组索引不同，所以查找的时候要计算这个偏移值
    ```js
    function list2tree(list, startIdx) {
        list.sort((m, n) => m-n);
        let root;
        list.forEach((item) => {
            if (item.pid === 0) {
...
function tree2list (tree) {
    ...
    if (tree.children.length > 1) {
        arr = tree.children.reduce((a, c) => {
            return [...tree2list(a), ...tree2list(c)];
        })
    }
