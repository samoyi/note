...
4. 如果没有对结构中的成员初始化，访问成员时会得到无意义的值；另外，如果直接声明一个结构指针，并不会自动为该指针分配结构体所需要的内存
    ```cpp
    typedef struct {
        long long a;
        int b;
    ...
    Test_Struct s1 = {1ll, 2, 3};
    Test_Struct s2;
    Test_Struct* p1;

    printf("%d %d %d\n", sizeof(s1), sizeof(s2), sizeof(p1));     // 16 16 4

    printf("s1: %lld %d %d\n", s1.a, s1.b, s1.c);    // s1: 1 2 3
    printf("s2: %lld %d %d\n", s2.a, s2.b, s2.c);    // s2: 27583486860722174 1976594221 4202480
    printf("p1: %lld %d %d\n", p1->a, p1->b, p1->c); // p1: 3660600626398544 1628182621 31232

    s2.a = 11ll;
    s2.b = 22;
    s2.c = 33;
    printf("s2: %lld %d %d\n", s2.a, s2.b, s2.c);    // s2: 11 22 33

    p1->b = 11; // Segmentation fault
    ```
