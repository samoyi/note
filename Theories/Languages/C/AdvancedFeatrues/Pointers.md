...
- [Pointers](#pointers)
    ...
    - [指针作为参数](#指针作为参数)
        - [例](#例)
        - [指针仍然是按值传递的](#指针仍然是按值传递的)
        - [用 `const` 保护参数](#用-const-保护参数)
            - [其他两种使用 `const` 的方式](#其他两种使用-const-的方式)
    - [指针作为返回值](#指针作为返回值)
...
4. 例如下面的一段程序
    <img src="./images/18.png" width="800" style="display: block; margin: 5px 0 10px;" />
    比如说，计算机可能将栈中 4 100 000号存储器单元分配给变量 `x`。如果把 4 赋给变量 `x`，计算机就会把 4 保存在 4 100 000 号单元。
...
7. 可以理解为：因为 `p` 是指针，所以 `const p` 表示指针不能变动，也就是不能指向其他地方；而 `*p` 是指针指向的对象，所以 `const char* p` 表示 `*p` 这个对象不能改变。
 
#### 其他两种使用 `const` 的方式
1. 前面说的 `const int *p` 是保护了指针指向 `int` 对象。
2. 如果希望保护指针，就需要把 `const` 放在指针变量前面，即 `int * const p`
    ...
    ```
3. 如果需要同时保护指针和它所指向的对象，那就要两个都加 `const`
    ```cpp
