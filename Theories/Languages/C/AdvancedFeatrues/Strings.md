...
- [Strings](#strings)
    ...
    - [字符串变量](#字符串变量)
        - [初始化字符串变量](#初始化字符串变量)
            - [《嗨翻C语言》中的解释：初始化字符串变量的复制过程](#嗨翻c语言中的解释初始化字符串变量的复制过程)
        - [字符数组与字符指针](#字符数组与字符指针)
    - [字符串的读和写](#字符串的读和写)
...
6. 既然字符串字面量是作为数组来存储的，那么编译器会把它看作是 `char*` 类型的 **指针**。例如，`printf` 函数和 `scanf` 函数都接收 `char*` 类型的值作为它们的第一个参数。思考下面的例子：
    ```cpp
    printf("abc");
    ```
...
1. 字符串变量可以在声明时进行初始化：
    ...
    ```
2. 编译器将把字符串 `"June 14"` 中的字符 **复制**（TODO，是不是复制？下面有两种解释） 到数组 `date1` 中，然后追加一个空字符从而使 `date1` 可以作为字符串使用。`date1` 将如下所示
    <img src="./images/29.png" width="400" style="display: block; margin: 5px 0 10px;" />
3. 根据《C语言程序设计：现代方法（第2版）》：`"June 14"` 看起来是字符串字面量，但其实不然。C 编译器会把它看成是数组初始化式的缩写形式。也就是说不是先创建字符串字面量再复制到数组里。实际上，我们可以写成：
    ```cpp
...
4. 调用时，`scanf` 函数会跳过起始的若干个空白字符，然后读入字符并存储到 `str` 中，直到遇到空白字符为止
    ...
    scanf("%d%s%d", &i, s, &j); // 输入 "12abc34  56def78"
...
5. 或者，如果你只想把输入作为字符串，而不需要 `scanf` 那样把输入转换为比如数值的格式化功能，那么可以直接使用更安全的 `fgets` 方法
    ```cpp
    char food[5];
    fgets(food, sizeof(food), stdin); // 输入 "abcde"
    printf("%s\n", food); // abcd
    ```
    `fgets` 最多会读取第二个参数减一的字符，因此这里保证在 `food` 中最后一位保存字符串结尾的空字符。
...
11. `read_line` 函数现在不具备检测 `getchar` 函数读入字符是否失败的功能。如果因为错误或到达文件尾而不能读入字符，`getchar` 函数会返回 `int` 类型的值 `EOF`。下面是改进后的
    ...
    ```
12. 这里一样，字符数超过数组长度依然可以使用，但会使用到了数组外面的内存，很有可能发生错误。所以 `read_line` 的第二个参数最大值不应该超过数组长度减一。
...
4. 下面用指针算术运算代替数组取下标来重新编写 `count_spaces` 函数。这次不再需要变量 `i`，而是利用 `s` 自身来跟踪字符串中的位置。通过对 `s` 反复进行自增操作，`count_spaces` 函数可以逐个访问字符串中的字符：
    ```cpp
    // 参数定义为 const char s[] 也是一样的，数组作为参数也是传递指针副本
    int count_spaces(const char *s)
    {
        int count = 0;
