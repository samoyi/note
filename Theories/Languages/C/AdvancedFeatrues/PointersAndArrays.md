...
4. 假设调用 `find_largest` 函数如下：
    ...
    ```
5. 这个调用会把指向数组 `b` 第一个元素的指针赋值给 `a`，数组本身并没有被复制。现在在函数里 `a` 是数组第一个元素的指针了，但为什么函数里面用的是 `a[0]` 而不是 `*a` 呢？因为正如下一节要说的，不仅可以用数组名作为指针，C 语言还允许反过来把指针看作数组名来进行取下标操作。
6. 所以下面的方式传单和传递数组名的方式是一个效果
    ```cpp
