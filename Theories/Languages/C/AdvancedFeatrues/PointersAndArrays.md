...
- [Pointers and Arrays](#pointers-and-arrays)
    ...
    - [用数组名作为指针](#用数组名作为指针)
        - [数组型实际参数（改进版）](#数组型实际参数改进版)
        - [把数组型形式参数声明为 `*a` 和 `a[]` 哪种风格更好呢？](#把数组型形式参数声明为-a-和-a-哪种风格更好呢)
        - [用指针作为数组名](#用指针作为数组名)
        - [一个奇怪的例子](#一个奇怪的例子)
    - [指针和多维数组](#指针和多维数组)
        - [处理多维数组的元素](#处理多维数组的元素)
        - [处理多维数组的行](#处理多维数组的行)
        - [处理多维数组的列](#处理多维数组的列)
        - [用多维数组名作为指针](#用多维数组名作为指针)
    - [C99 中的指针和变长数组](#c99-中的指针和变长数组)
    - [习题](#习题)
...
### 数组型实际参数（改进版）
1. 数组名在传递给函数时，总是被视为指针。思考下面的函数，这个函数会返回整型数组中最大的元素：
    ```cpp
    ...
    ```
2. 假设调用 `find_largest` 函数如下：
    ```cpp
    largest = find_largest (b, N);
    ```
3. 这个调用会把指向数组 `b` 第一个元素的指针赋值给 `a`，数组本身并没有被复制。现在在函数里 `a` 是数组第一个元素的指针了，但为什么函数里面用的是 `a[0]` 而不是 `*a` 呢？因为正如下一节要说的，不仅可以用数组名作为指针，C 语言还允许反过来把指针看作数组名来进行取下标操作。
4. 所以下面的方式传单和传递数组名的方式是一个效果
    ```cpp
    largest = find_largest (&b[0], N);
    ```
5. 在给函数传递普通变量时，变量的值会被复制；任何对相应的形式参数的改变都不会影响到变量。反之，因为没有对数组本身进行复制，所以作为实际参数的数组是可能被改变的。
6. 为了指明数组型形式参数不会被改变，可以在其声明中包含单词 `const`：
    ```cpp
...
7. 给函数传递数组所需的时间与数组的大小无关。因为没有对数组进行复制，所以传递大数组不会产生不利的结果。
8. 如果需要，可以把数组型形式参数声明为指针。例如，可以按如下形式定义 `find_largest` 函数：
    ```cpp
    int find_largest(int *a, int n)
    {
        ...
...
9. 对于形式参数而言，声明为数组跟声明为指针是一样的；但是对变量而言，声明为数组跟声明为指针是不同的。声明
    ```cpp
    int a[10];
    ```
    ...
    ```
    只会导致编译器为一个指针变量分配空间。
10. 既然传参时可以传递数组第一项的指针，那也可以传递其他项的指针。假设希望用 `find_largest` 函数来定位数组 `b` 中某一部分的最大元素，比如说元素 `b[5]`,…,`b[14]`。调用 `find_largest` 函数时，将传递 `b[5]` 的地址和数 `10`，表明希望 `find_largest` 函数从 `b[5]` 开始检查 10 个数组元素：
    ```cpp
...
2. 看起来，数组名本身就是指向数组第一项的指针，两者似乎就是一个东西。但从技术上说，数组的名字不是指针，只是 C 语言编译器会在需要时把数组的名字转换为指针。使用 `find_largest` 举个例子来看出区别
    ```cpp
...
2. 使用指针时可以利用这一布局特点。如果使指针 `p` 指向二维数组中的第一个元素（即 0 行 0 列的元素），就可以通过重复自增p的方法访问数组中的每一个元素。
...
1. 下面的循环对数组 `a` 的第 `i` 列清零：
    ...
    for (p = &a[0]; p < &a[NUM_ROWS]; p++)
        (*p)[i] = 0;
    ```
2. 这里把p声明为指向长度为 `NUM_COLS` 的整型数组的指针。表达式 `p++` 把 `p` 移到下一行的开始位置。
3. 在表达式 `(*p)[i]` 中，`*p` 代表 `a` 的一整行，因此 `(*p)[i]` 选中了该行第 `i` 列的那个元素。
4. `(*p)[i]` 中的括号是必要的，因为编译器会将 `*p[i]` 解释为 `*(p[i])`。
...
7. 我们把尝试把 `a`（数组的地址）作为 `find_largest` 函数的第一个实际参数，`NUM_ROWS * NUM_COLS`（数组a中的元素总数量）作为第二个实际参数：
    ```cpp
