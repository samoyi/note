...
- [Basic Types](#basic-types)
    ...
    - [字符类型](#字符类型)
        - [字符操作](#字符操作)
        - [有符号字符和无符号字符](#有符号字符和无符号字符)
        - [算术类型](#算术类型)
            - [C89 中的算术类型](#c89-中的算术类型)
            - [C99 中的算术类型](#c99-中的算术类型)
        - [转义序列](#转义序列)
        - [用 `scanf` 和 `printf` 读/写字符](#用-scanf-和-printf-读写字符)
        - [用 `getchar` 和 `putchar` 读/写字符](#用-getchar-和-putchar-读写字符)
            - [混合使用 `getchar` 函数和 `scanf` 函数的问题](#混合使用-getchar-函数和-scanf-函数的问题)
    - [类型转换](#类型转换)
        - [常用算术转换](#常用算术转换)
            - [转换规则](#转换规则)
                - [任一操作数的类型是浮点类型的情况](#任一操作数的类型是浮点类型的情况)
                - [两个操作数的类型都不是浮点类型的情况](#两个操作数的类型都不是浮点类型的情况)
                - [有符号操作数和无符号操作数组合](#有符号操作数和无符号操作数组合)
        - [赋值过程中的转换](#赋值过程中的转换)
        - [C99 中的隐式转换](#c99-中的隐式转换)
        - [强制类型转换](#强制类型转换)
    - [类型定义](#类型定义)
        - [类型定义的优点](#类型定义的优点)
        - [类型定义和可移植性](#类型定义和可移植性)
...
2. 今最常用的字符集是 `ASCII`（美国信息交换标准码）字符集，它用 7 位代码表示 128 个字符。在 ASCII 码中，数字 0~9 用`0110000`~`0111001` 码来表示，大写字母 A~Z 用 `1000001~1011010` 码表示。ASCII 码常被扩展用于表示 256 个字符，相应的字符集 Latin-1 包含西欧和许多非洲语言中的字符。
...
7. 字符拥有和数相同的属性，这一事实会带来一些好处。例如，可以让 `for` 语句中的控制变量遍历所有的大写字母：
    ...
    for (ch = 'A'; ch <= 'Z'; ch++)...
    ```
8. 另一方面，以数的方式处理字符可能会导致编译器无法检查出来的多种编程错误，还可能会导致我们编写出诸如 `'a' * 'b' / 'c'` 这类无意义的表达式。TODO，怎么无意义了。此外，这样做也可能会妨碍程序的可移植性，因为程序可能会基于一些对字符集的假设。（例如，上述 `for` 循环假设从字母 A 到字母 Z 的代码都是连续的。）
...
1. 如果在变量中只存储7位的字符，那么不需要考虑，因为符号位将为零。但是，如果计划存储8位字符，那么将希望变量是 `unsigned char` 类型。思考下面的例子：
    ```cpp
    ch = '\xdb';
    ```
