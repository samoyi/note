...
* 1. [整数类型](#)
	* 1.1. [有符号整数和无符号整数](#-1)
	* 1.2. [6 种整数类型](#-1)
		* 1.2.1. [C99 中的整数类型](#C99)
	* 1.3. [整数常量](#-1)
		* 1.3.1. [C99 中的整数常量](#C99-1)
	* 1.4. [整数溢出](#-1)
	* 1.5. [读/写整数](#-1)
* 2. [浮点类型](#-1)
	* 2.1. [IEEE 浮点标准](#IEEE)
	...
	* 2.3. [十六进制的浮点常量](#-1)
	* 2.4. [读/写浮点数](#-1)
		* 2.4.1. [为什么使用 `%lf` 读取 `double` 类型的值，而用 `%f` 进行显示呢？](#lfdoublef)
* 3. [字符类型](#-1)
	* 3.1. [字符操作](#-1)
	* 3.2. [有符号字符和无符号字符](#-1)
	* 3.3. [算术类型](#-1)
		* 3.3.1. [C89 中的算术类型](#C89)
		* 3.3.2. [C99 中的算术类型](#C99-1)
	* 3.4. [转义序列](#-1)
	* 3.5. [用 `scanf` 和 `printf` 读/写字符](#scanfprintf)
	* 3.6. [用 `getchar` 和 `putchar` 读/写字符](#getcharputchar)
	* 3.7. [读入字符时对多字符（包括换行符）的处理](#-1)
	* 3.8. [什么时候需要考虑字符变量是有符号的还是无符号的  不懂](#-1)
* 4. [类型转换](#-1)
	* 4.1. [常用算术转换](#-1)
		* 4.1.1. [转换规则](#-1)
	* 4.2. [赋值过程中的转换](#-1)
	* 4.3. [C99 中的隐式转换](#C99-1)
	* 4.4. [强制类型转换](#-1)
* 5. [类型定义](#-1)
	* 5.1. [类型定义的优点](#-1)
	* 5.2. [类型定义和可移植性](#-1)
...
<!-- vscode-markdown-toc-config
	...
	autoSave=true
	/vscode-markdown-toc-config -->
<!-- /vscode-markdown-toc --># Basic Types
...
####  1.3.1. <a name='C99-1'></a>C99 中的整数常量
...
####  2.4.1. <a name='lfdoublef'></a>为什么使用 `%lf` 读取 `double` 类型的值，而用 `%f` 进行显示呢？
...
####  3.3.2. <a name='C99-1'></a>C99 中的算术类型
* 整数类型：
    * 字符类型（`char`）；
...
###  3.5. <a name='scanfprintf'></a>用 `scanf` 和 `printf` 读/写字符
...
###  3.6. <a name='getcharputchar'></a>用 `getchar` 和 `putchar` 读/写字符
...
###  3.7. <a name='-1'></a>读入字符时对多字符（包括换行符）的处理
...
###  3.8. <a name='-1'></a>什么时候需要考虑字符变量是有符号的还是无符号的  不懂
...
2. 计算机可能可以直接将两个 16 位整数相加，但是不能直接将 16 位整数和 32 位整数相加，也不能直接将 32 位整数和 32 位浮点数相加。
