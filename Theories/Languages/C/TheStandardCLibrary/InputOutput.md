...
####  2.3.1. <a name='fclose'></a>`fclose` 的必要性
...
####  2.5.2. <a name='-1'></a>程序　检查文件是否可以打开
...
###  7.4. <a name='fgetposfsetpos'></a>`fgetpos` 和 `fsetpos`
...
###  7.5. <a name='-1'></a>当正在读取用户输入时，如何跳过当前输入行中剩下的全部字符
...
####  8.1.2. <a name='snprintf'></a>`snprintf`
